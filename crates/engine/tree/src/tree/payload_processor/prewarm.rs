...
use reth_trie_common::{bal::BalAccountState, MultiProofTargetsV2};
use std::sync::{
    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
    ...
    fn send_bal_hashed_state(
        ...
        if !account_changes.storage_changes.is_empty() {
            ...
            storage_map
                .storage
                .extend(reth_trie_common::bal::hashed_storage_changes(account_changes));
        ...
        let existing_account = if account_state.needs_parent_account() {
            if provider.is_none() {
        ...
        // `None` and the empty-code hash give the same trie leaf and the same `is_empty` result.
