...
//!
//! A snap range response proves a consecutive run of leaves with the boundary trie nodes that
//! connect them to the rest of the trie. Checking only the first and last leaf is not enough: a
//! peer could omit a leaf in the middle and still prove both endpoints. The verifier below
//! reconstructs the trie root from the returned leaves plus the proof subtrees that fall outside
//! the proven range, so any omission changes the root.
...
/// Number of nibbles in a hashed trie key.
const KEY_NIBBLES: usize = 64;

/// Upper bound used when a range has no explicit right boundary.
...
/// Verifies that `leaves` are complete from `origin` through the last returned leaf, or through
/// the end of the trie when no leaves were returned.
///
/// An empty `proof` asserts that `leaves` covers the whole trie from `origin`, which is how
/// servers answer a zero-origin range that exhausted the trie.
...
/// Walks a child reference, keeping only the parts of the trie outside `[left, right]`.
///
/// Subtrees fully inside the range are dropped: the response's own leaves stand in for them, which
/// is what makes an omitted leaf change the reconstructed root.
fn visit_reference(
    prefix: Nibbles,
    reference: &RlpNode,
    left: &Nibbles,
...
fn frontier_root(mut frontier: Vec<FrontierEntry>) -> Result<B256, RangeProofError> {
    frontier.sort_unstable_by_key(FrontierEntry::path);
...
mod tests {
    ...
    #[test]
    fn proof_free_range_rejects_omitted_tail_leaf() {
        let leaves = vec![(b256(1), value(1)), (b256(2), value(2)), (b256(3), value(3))];
        let (root, _) = build_proof(&leaves, &[]);

        assert!(matches!(
            verify_range_proof(root, B256::ZERO, leaves[..2].to_vec(), &[]),
            Err(RangeProofError::RootMismatch { .. })
        ));
    }
    ...
    #[test]
    fn leaves_before_origin_are_rejected() {
        let leaves = vec![(b256(1), value(1)), (b256(2), value(2))];
        let (root, proof) = build_proof(&leaves, &[b256(2)]);

        assert!(matches!(
            verify_range_proof(root, b256(2), leaves, &proof),
            Err(RangeProofError::LeafBeforeOrigin { .. })
        ));
    }

    #[test]
    fn empty_root_accepts_only_empty_range() {
        verify_range_proof(EMPTY_ROOT_HASH, B256::ZERO, std::iter::empty::<(B256, Vec<u8>)>(), &[])
            .unwrap();

        assert!(matches!(
            verify_range_proof(EMPTY_ROOT_HASH, B256::ZERO, vec![(b256(1), value(1))], &[]),
            Err(RangeProofError::RootMismatch { .. })
        ));
    }
}
