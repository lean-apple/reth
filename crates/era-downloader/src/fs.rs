...
const ERAE_FILE_EXTENSION: &str = "erae";

/// Creates a new ordered asynchronous [`Stream`] of ERAE files read from `dir`.
pub fn read_dir(
    ...
    let mut entries = fs::read_dir(dir)?
        .filter_map(|entry| {
            (|| {
                ...
                if path.extension() == Some(ERAE_FILE_EXTENSION.as_ref()) &&
