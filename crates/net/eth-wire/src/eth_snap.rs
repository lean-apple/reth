...
//! side-channel over a single `RLPx` connection. It is a purpose-built two-protocol stream, not the
//! general-purpose [`RlpxSatelliteStream`](crate::multiplex::RlpxSatelliteStream): `eth` is always
//! the first shared capability (relative offset `0`), so only `snap/2` message ids need masking,
//! which keeps the demultiplexing small enough to own directly.
...
/// An `eth`-only byte view bridged to the shared wire through channels.
///
/// `eth` is the first shared capability (relative offset `0`), so no id masking is required: bytes
