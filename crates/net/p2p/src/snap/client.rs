...
pub struct SnapRequestOptions {
    /// Where the request is placed relative to other queued work.
    pub priority: Priority,
    /// Peers that already failed this logical request and must not receive it again.
    pub excluded_peers: Vec<PeerId>,
}

impl SnapRequestOptions {
    /// Creates options at `priority` without excluding any peer.
    pub const fn new(priority: Priority) -> Self {
        Self { priority, excluded_peers: Vec::new() }
    }

    /// Excludes peers already tried for the same logical request.
    pub fn excluding(mut self, peers: Vec<PeerId>) -> Self {
