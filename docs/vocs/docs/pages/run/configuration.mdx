...
The ERA stage configures pre-synced ERAE data ingestion, either from a local directory or a remote host.

```toml
[stages.era]
# Use a local directory containing ERAE files (conflicts with `url`)
path = "/path/to/erae"
# Or download ERAE files from a host (conflicts with `path`)
# url = "https://example.com/erae/"
