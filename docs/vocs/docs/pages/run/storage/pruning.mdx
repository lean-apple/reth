...
-   History-expiry node - uses full-node state pruning while retaining 1,056,768 blocks of
    bodies and receipts.
...
Use `--history-expiry` to retain the CL block-serving window without changing the existing full
node preset:
...
This mode uses the same state-history settings as `--full`, but retains the last 1,056,768 blocks
(33,024 epochs) of bodies and receipts. Its receipt window is about 105 times larger than `--full`,
with correspondingly higher receipt download and storage requirements.

Enabling the mode cannot restore history that an existing full node has already pruned. A
history-expiry snapshot provides a replacement data directory with the required history:

```bash
reth download --history-expiry --datadir /path/to/reth-history-expiry -y
reth node --history-expiry --datadir /path/to/reth-history-expiry
```
